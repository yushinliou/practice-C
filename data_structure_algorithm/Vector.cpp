...
void Test3(); // Max, Min
void Test4(); // Median, Sort wa
void Test5(); // Sum, Average (要加int再除以size_t之前)
void Test6(); // TrimZeroes re
void Test7(); // All re
...
void Test1() {
    ...
    // Part 3: Median, Sort
    {
        ...
        std::cout << "3.1) " << a.Median() << std::endl; // err
        std::cout << "3.2) " << a << std::endl;
        std::cout << "3.3) " << a.Sort() << std::endl; // err

        NumericArray<double> b{1, 0, 7, 3, 2, 9, 5, -3, 0};
        std::cout << "3.4) " << b.Median() << std::endl; // err
        std::cout << "3.5) " << b << std::endl;
        std::cout << "3.6) " << b.Sort() << std::endl; // err
    }
...
NumericArray<T>& NumericArray<T>::Sort(){
    ...
    for(int i = 0; i < repr_.Size(); i++){
        int min_index = i;
        for (int j = i + 1; j < repr_.Size(); j++){
            if (repr_.At(j) < repr_.At(min_index)){
                min_index = j;
            }
        }
        tmp = repr_.At(min_index);
        repr_[min_index] = repr_[i];
        repr_[i] = tmp;
    }
    return *this;
}
